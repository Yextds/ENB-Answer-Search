...
const builtInCssClasses: FilterDisplayManagerCssClasses = {
  ...
  container___mobileFiltersExpanded:
    "py-5 px-4 flex flex-col overflow-x-hidden fixed bg-white inset-0 z-10",
...
export default function FilterDisplayManager({
  ...
  if (pageView === PageView.Desktop) {
    ...
    const childrenWithUpdatedDividers = recursivelyMapChildren(
      children,
      (child) => {
        if (
          child.type !== Facets &&
          child.type !== StaticFilters &&
          child.type !== Divider
        ) {
          return child;
        }
 ...
 */
function customizeComponentDivider(
  child: ReactElement,
