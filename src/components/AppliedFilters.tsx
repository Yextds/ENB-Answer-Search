...
const builtInCssClasses: AppliedFiltersCssClasses = {
  ...
  nlpFilter:
    "border rounded-3xl px-3 py-1.5 text-sm font-medium italic text-gray-800 mr-2 mb-4",
  removableFilter:
    "flex items-center border rounded-3xl px-3 py-1.5 text-sm font-medium text-gray-900 mr-2 mb-4",
...
export default function AppliedFilters(
  ...
  const nlpFilters =
    useSearchState((state) => state.vertical.appliedQueryFilters) || [];
  ...
  const appliedFilters = groupedFilters.flatMap(
    (groupedFilters) => groupedFilters.filters
  );
  ...
  function RemovableFilter({
    ...
    const onRemoveFacetOption = () => {
      ...
      if (isNearFilterValue(value)) {
        console.error(
          "A Filter with a NearFilterValue is not a supported RemovableFilter."
        );
        return;
      }
    ...
    const onRemoveFilter =
      filter.filterType === "FACET"
        ? onRemoveFacetOption
        : onRemoveStaticFilterOption;
  ...
  return (
    <>
      {displayableFilters.length > 0 && (
        <div
          className={cssClasses.appliedFiltersContainer}
