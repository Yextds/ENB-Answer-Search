 ...
 */
export function EnbCards(props: StandardCardProps): JSX.Element {
  ...
  */
  ...
  const Links : any = result.rawData;
  
  const NewsName =  Links.name;
  const NewsDesc = Links.c_description;
  const NewsDate = Links.c_date;
  
  console.log(NewsDate, NewsDesc);
  ...
  return (
    <>
      <div className={cssClasses.container}>
        <div>
          {/* <a href={LinkLandingPage}> */}
            <h3 className={cssClasses.header}>{NewsName}</h3>
          {/* </a> */}
          <p>{NewsDate}</p>
          <p>{NewsDesc}</p>
        </div>
